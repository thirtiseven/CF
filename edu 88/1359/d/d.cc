#include <iostream>
#include <algorithm>

const int maxn = 100009;
int A[maxn], sum[maxn << 2], Add[maxn << 2];

void pushup(int rt) {
	sum[rt] = std::max(sum[rt << 1], sum[rt << 1 | 1]);
}

void pushdown(int rt, int l, int r) {
	if (Add[rt]) {
		int mid = (l + r) >> 1;
		Add[rt << 1] += Add[rt];
		Add[rt << 1 | 1] += Add[rt];
		sum[rt << 1] += (mid - l + 1)*Add[rt];
		sum[rt << 1 | 1] += (r - mid)*Add[rt];
		Add[rt] = 0;
	}
}

void build(int l, int r, int rt) {
	if (l == r) {
		sum[rt] = A[l];
		return;
	}
	int mid = (l + r) >> 1;
	build(l, mid, rt << 1);
	build(mid + 1, r, rt << 1 | 1);
	pushup(rt);
}

//点修改
void update(int index, int val, int l, int r, int rt) {
	if (l == r) {
		sum[rt] = val;
		return;
	}
	int mid = (l + r) >> 1;
	if (index <= mid)update(index, val, l, mid, rt << 1);
	else update(index, val, mid + 1, r, rt << 1 | 1);
	pushup(rt);
}

//区间查询
int query(int L, int R, int l, int r, int rt) {
	if (L <= l && R >= r) {
		return sum[rt];
	}
	pushdown(rt, l, r);
	int mid = (l + r) >> 1;
	int ret = -1;
	if (L <= mid) ret = std::max(query(L, R, l, mid, rt << 1), ret);
	if (R > mid) ret = std::max(query(L, R, mid + 1, r, rt << 1 | 1), ret);
	return ret;
}

int n, dp[maxn], pos[maxn], next[maxn], pre[maxn];

int main(int argc, char *argv[]) {  
	std::ios::sync_with_stdio(false);
	std::cin.tie(0); 
	std::cin >> n;
	pre[0] = 0;
	for (int i = 1; i <= n; i++) {
		std::cin >> A[i];
		pre[i] = A[i] + pre[i-1];
	}
	next[n+1] = -1, A[n+1] = -1;
	for (int i = n; i >= 1; i--) {
		if (A[i+1] > 0) {
			next[i] = i+1;
		} else {
			next[i] = next[i+1];
		}
	}
	build(1, n, 1);
	dp[0] = 0, pos[0] = 1;
	int ans = 0;
	for (int i = 1; i <= n; i++) {
		if (dp[i-1] > 0) {
			dp[i] = dp[i-1] + A[i];
			pos[i] = pos[i-1];
		} else {
			dp[i] = A[i];
			pos[i] = i;
		}
		int temp = dp[i] - query(pos[i], i, 1, n, 1);
//		std::cout << pos[i] << ' ' << i << ' ' << temp << '\n';
		if (temp > ans) {
			ans = temp;
		}
		if (A[pos[i]] == query(pos[i], i, 1, n, 1) && next[pos[i]] < i) {
			ans = std::max(ans, pre[i]-pre[next[pos[i]]-1]-query(next[pos[i]], i, 1, n, 1));
		}
	}
	std::cout << ans << '\n';
}